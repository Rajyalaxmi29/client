...
export default function Upload() {
  ...
  // Handle upload (connect this to your backend later)
  const handleUpload = async () => {
    if (!file) return alert("Please select a file before uploading.");
    // Here you can send `file` and `selectedMoods` to your backend API
    alert(
      `Uploading: ${file.name}\nMoods: ${selectedMoods
        .map((m) => m.label)
        .join(", ")}`
  ...
  return (
    <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-8 rounded-xl shadow max-w-lg mx-auto mt-10">
      <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-100">
        Upload Your Outfit
      </h2>
      <div
        className="flex flex-col items-center justify-center border-2 border-dashed border-pink-400 rounded-lg p-6 cursor-pointer hover:bg-pink-50 dark:hover:bg-gray-800 transition mb-4"
